...
const RoadmapStep = ({
  ...
  step: (typeof scheduleSteps)[number];
  index: number;
}) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "absolute left-4 top-6 z-10 h-5 w-5 -translate-x-1/2 rounded-full border-4 border-background bg-primary shadow-elegant md:left-1/2",
          inView && "animate-pulse-glow",
        )}
      />

      {/* Card — left col on even, right col on odd */}
      <div
        className={cn(
          "ml-12 md:ml-0",
...
const BGMI = () => {
  ...
  for (let i = 1; i <= 4; i++) {
    teamMembers.push({
      player: i,
      ign: inputData[`ign_${i}`],
      bgmiId: inputData[`bgmiId_${i}`],
      isLeader: i === 1,
    });
  }

  inputData.teamMembers = JSON.stringify(teamMembers);
  inputData.amount = "1000";
  inputData.payment_status = "pending";

  try {
  const response = await fetch(
    "https://ssccmangalore.co.in/submit_bgmi.php",
    {
      method: "POST",
      headers: {
  ...
  try {
    ...
    console.log("BGMI API Response:", result);
  } catch (err) {
  ...
  if (result && result.success === true) {
    toast({
      title: "Success!",
      description: result.message || "Registration submitted successfully!",
    });
  ...
  return (
    <div className="bg-secondary text-secondary-foreground">
      ...
      {/* Player image showcase with animation */}
      <section className="bg-background py-16 text-foreground md:py-24">
        <div
          ref={playerRef}
          className="container mx-auto grid items-center gap-10 px-4 md:grid-cols-2"
        >
          <div
            className={cn(
              "relative transition-all duration-1000",
              playerIn ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10",
            )}
          >
            <div className="absolute -inset-3 rounded-3xl bg-gradient-hero opacity-30 blur-2xl" />
            <img
              src={bgmiPlayer}
              alt="Pro BGMI gamer playing on mobile in neon esports setup"
              loading="lazy"
              width={1024}
              height={1024}
          ...
          >
            ...
            <h2 className="mt-3 font-display text-3xl font-extrabold md:text-5xl">
              Drop In. Outplay. <span className="text-gradient-hero">Conquer.</span>
            </h2>
            <p className="mt-4 text-muted-foreground">
              Squad up with your best four and battle for glory at Mangalore's biggest BGMI championship.
              Compete against the region's top players, prove your skill on Erangel, and lift the trophy.
            </p>
      ...
      {/* Prizes with count-up */}
      <section className="py-16">
        <div className="container mx-auto px-4">
          <h2 className="text-center font-display text-3xl font-bold md:text-4xl">
            Win Big — Prize Pool{" "}
            <CountUp end={21000} prefix="₹" className="text-accent" />
          </h2>
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {prizes.map((x, i) => (
              <PrizeCard key={x.t} prize={x} index={i} />
            ))}
          </div>
      ...
      <section className="bg-background py-16 text-foreground">
        <div className="container mx-auto px-4">
          ...
          <h2 className="mt-20 text-center font-display text-3xl font-bold">
            Match <span className="text-gradient-hero">Roadmap</span>
          </h2>
          <p className="mt-2 text-center text-muted-foreground">
            Your journey from registration to the grand finals
          </p>
          ...
          <div className="relative mx-auto mt-12 max-w-4xl">
            ...
            <span
              aria-hidden
              className="absolute left-4 top-0 h-full w-0.5 bg-gradient-to-b from-primary via-accent to-primary md:left-1/2 md:-translate-x-1/2"
            />
            <div className="space-y-10">
              {scheduleSteps.map((step, i) => (
                <RoadmapStep key={step.title} step={step} index={i} />
              ))}
            </div>
          </div>
        </div>
      ...
      <section id="register" className="py-16">
        <div className="container mx-auto max-w-4xl px-4">
          ...
          <p className="mt-2 text-center text-secondary-foreground/70">Enter details for all 4 squad members</p>
          <form onSubmit={handleSubmit} className="mt-10 space-y-8">
  {[1, 2, 3, 4].map((n) => (
    <Card
      key={n}
      className="bg-background/5 p-6 text-secondary-foreground border-white/10"
    >
      <h3 className="font-display text-lg font-bold">
        <span className="mr-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-accent text-accent-foreground text-sm">
          {n}
        </span>
          ...
          <>
            <div>
              ...
              <Input
                required
                name="squadName"
                className="mt-1 bg-background/10 border-white/20"
                placeholder="e.g. Alpha Predators"
              />
            </div>
