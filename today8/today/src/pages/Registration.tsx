...
const Registration = () => {
  ...
  toast({
    title: "Error",
    description: "Please select both sport and gender",
    variant: "destructive"
  });
  return;
}
 setSubmitting(true);
    ...
    const inputData: any = Object.fromEntries(formData.entries());
    ...
    inputData.college_name = inputData.college_name || '';
    inputData.email = inputData.email || '';
    inputData.phone = inputData.phone || '';
    inputData.address = inputData.address || '';

    if (selectedSport === 'tugofwar') {
      toast({
        title: "Tug of War Reminder",
        description: "Ensure team total weight ≤480kg to avoid disqualification.",
        duration: 5000
      });
    }

    try {
  const response = await fetch('/sports_register.php', {
    method: 'POST',
    headers: { 
  ...
  if (result.success) {
    toast({ title: "Registration Success!", description: result.message });
  ...
  toast({ 
    title: "Network Error", 
    description: "Ensure PHP backend is running",
    variant: "destructive" 
  });
} finally {
  ...
  size="lg" 
  disabled={submitting}
  className="md:col-span-2 bg-gradient-hero shadow-elegant"
>
      ...
      <AlertDialog open={feeOpen} onOpenChange={setFeeOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
{`${config?.label} registration is open. Please complete the form.`}
              {selectedSport === 'tugofwar' && (
                <span className="mt-2 block text-destructive font-semibold">
                  ⚠️ Weight Limit: Team total must be under 480kg. Exceeding teams will be disqualified.
                </span>
              )}
              {config && config.players > 1 && (
                <span className="mt-2 block">
  {`Please enter the names of all ${config.players} team members below.`}
</span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
